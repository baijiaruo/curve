...
int S3ClientImpl::Upload(std::string name, const char* buf, uint64_t length) {
    ...
    LOG(INFO) << "upload start, aws_key:" << aws_key
              << ",length:" << length << ",data len:" << data.length();
    ret = s3Adapter_.PutObject(aws_key, data);
    LOG(INFO) << "upload end, ret:" << ret;
    return ret;
}

int S3ClientImpl::Download(std::string name, char* buf,
    ...
    LOG(INFO) << "download start, aws_key:" << aws_key
